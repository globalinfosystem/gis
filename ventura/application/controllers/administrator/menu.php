                        ...
                        if (isset($l['children'])) {
                            ...
                            foreach ($l['children'] as $l2) {
                                if (isset($l2['children'])) {
                                    $d = 0;
                                    foreach ($l2['children'] as $l23) {

                                        if (isset($l23['children'])) {
                                            $e = 0;
                                            foreach ($l23['children'] as $l234) {
                                                $level5[] = $l234['id'];
                                                $this->db->where(array('menu_id' => $l234['id']));
                                                $this->db->update('tb_menu', array('parent_id' => $l23['id'], 'ordering' => $e));
                                                $e++;
                                            }
                                        }

                                        $level4[] = $l23['id'];
                                        $this->db->where(array('menu_id' => $l23['id']));
                                        $this->db->update('tb_menu', array('parent_id' => $l2['id'], 'ordering' => $d));
                                        $d++;
                                    }
                                }
                                $level3[] = $l2['id'];
