    ...
    <div class="page-content-wrapper m-t">
        <div class="toolbar-line ">		
            <?php if ($this->access['is_add'] == 1) : ?>
                <a href="<?php echo site_url('administrator/districts/add') ?>" class="tips btn btn-xs btn-info"  title="<?php echo $this->lang->line('core.btn_new'); ?>">
            ...
            /*if ($this->access['is_excel'] == 1) :
                ?>	
                <a href="<?php echo site_url('administrator/districts/download') ?>" class="tips btn btn-xs btn-default" title="<?php echo $this->lang->line('core.btn_download'); ?>">
                    <i class="fa fa-download"></i>&nbsp;<?php echo $this->lang->line('core.btn_download'); ?></a>
                <?php
            endif;*/
            if ($this->session->userdata('gid') == 1) :
                ?>	
                <a href="<?php echo site_url('administrator/module/config/districts') ?>" class="tips btn btn-xs btn-default"  title="<?php echo $this->lang->line('core.btn_config'); ?>">
                    <i class="fa fa-cog"></i>&nbsp;<?php echo $this->lang->line('core.btn_config'); ?></a>
            <?php endif; ?>		

        </div>
        <?php echo $this->session->flashdata('message'); ?>
        <form action='<?php echo site_url('administrator/districts/destroy') ?>' class='form-horizontal' id ='SximoTable' method="post" >
                    ...
                    <tbody>
                        ...
                        <tr >
                            ...
                            <tr>
                                ...
                                <td width="50"><input type="checkbox" class="ids" name="id[]" value="<?php echo $row->districts_id ?>" />  </td>
                                    ...
                                    <?php if ($field['view'] == '1'): ?>
                                        <td>
                                            ...
                                            <?php else: ?>
                                               ...
                                               if($field['field']=='districts_access')
												{
												...
												else if($field['field'] == 'districts_allow_guest'){
												...
												else if($field['field'] == 'districts_content'){
													if(strlen($row->$field['field']) > 100) {
														    echo substr($row->$field['field'], 0, 100).'...';
													}
													else {
    														echo $row->$field['field'];
														}
												}
												else if($field['field'] == 'districts_created')
												{
													echo Sb_controller::show_date($row->$field['field']);
													
												}
												else if($field['field'] == 'districts_updated')
												{
                                    ...
                                    <div class="btn-group">		
                                        ...
                                        <ul  class="dropdown-menu  icons-left pull-right">					 	
                                            ...
                                            <?php if ($access['is_detail'] == 1) : ?>
                                                <li><a href="<?php echo site_url('administrator/districts/show/' . $row->districts_id) ?>"  class="tips "  title="view"><i class="fa  fa-search"></i> <?php echo $this->lang->line('core.btn_view'); ?> </a></li>
                                                <?php
                                            endif;
                                            if ($access['is_edit'] == 1) :
                                                ?>
                                                <li><a  href="<?php echo site_url('administrator/districts/add/' . $row->districts_id) ?>"  class="tips "  title="edit"> <i class="fa fa-edit"></i>  <?php echo $this->lang->line('core.btn_edit'); ?> </a> </li>
                                                <?php endif; ?>
...
<script>
    $(document).ready(function () {
        ...
        $('.do-quick-search').click(function () {
            $('#SximoTable').attr('action', '<?php echo site_url("administrator/districts/multisearch"); ?>');
            $('#SximoTable').submit();
        });
