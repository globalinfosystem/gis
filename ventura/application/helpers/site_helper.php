...
class SiteHelpers {
                            ...
                            if (count($menus3) > 0) {
                                ...
                                foreach ($menus3 as $row3) {
                                    ...
                                    if ($is_allow == 1) {
                                        ...
                                        $menus4 = self::nestedMenu($row3->menu_id, $position, $active);
                                        if (count($menus4) > 0) {
                                            $child_level_4 = array();
                                            foreach ($menus4 as $row4) {
                                                $p = json_decode($row4->access_data, true);
                                                if ($row4->allow_guest == 1) {
                                                    $is_allow = 1;
                                                } else {
                                                    $is_allow = (isset($p[$_this->session->userdata('gid')]) && $p[$_this->session->userdata('gid')] ? 1 : 0);
                                                }
                                                if ($is_allow == 1) {
                                                    $menu4 = array(
                                                        'menu_id' => $row4->menu_id,
                                                        'module' => $row4->module,
                                                        'menu_type' => $row4->menu_type,
                                                        'url' => $row4->url,
                                                        'menu_name' => $row4->menu_name,
                                                        'menu_icons' => $row4->menu_icons,
                                                        'childs' => array()
                                                    );
                                                    $menus5 = self::nestedMenu($row4->menu_id, $position, $active);
                                                    if (count($menus5) > 0) {
                                                        $child_level_5 = array();
                                                        foreach ($menus5 as $row5) {
                                                            $p = json_decode($row5->access_data, true);
                                                            if ($row5->allow_guest == 1) {
                                                                $is_allow = 1;
                                                            } else {
                                                                $is_allow = (isset($p[$_this->session->userdata('gid')]) && $p[$_this->session->userdata('gid')] ? 1 : 0);
                                                            }
                                                            if ($is_allow == 1) {
                                                                $menu5 = array(
                                                                    ...
                                                                    'module' => $row5->module,
                                                                    'menu_type' => $row5->menu_type,
                                                                    'url' => $row5->url,
                                                                    'menu_name' => $row5->menu_name,
                                                                    'menu_icons' => $row5->menu_icons,
                                                                    'childs' => array()
                                                                );
                                                                $child_level_5[] = $menu5;
                                                            }
                                                        }
                                                        $menu4['childs'] = $child_level_5;
                                                    }
                                                    $child_level_4[] = $menu4;
                                                }
                                            }
                                            $menu3['childs'] = $child_level_4;
                                        }
                                        $child_level_3[] = $menu3;
                                    }
                                }
    ...
    public static function nestedMenu($parent = 0, $position = 'top', $active = '1') {
        ...
        if (!empty($position) && $position == 'footer') {
            $Q = $_this->db->query("
			SELECT 
				tb_menu.*
			FROM tb_menu WHERE parent_id ='" . $parent . "' " . $active . "  AND position ='{$position}'
			GROUP BY tb_menu.menu_id ORDER BY tb_menu.menu_id DESC LIMIT 12			
			");
            return $Q->result();
        }
    ...
    public static function pageInfo($pageId = 1, $tableName = 'file') {
        $_this = & get_Instance();
        if (!empty($tableName)) {
            $Q = $_this->db->query("SELECT tb_page_" . $tableName . ".*	FROM tb_page_" . $tableName . " WHERE 1=1 AND tb_page_" . $tableName . "_status = 'Enabled' AND tb_page_" . $tableName . "_pageId = " . $pageId . "  GROUP BY tb_page_" . $tableName . ".tb_page_" . $tableName . "_id ORDER BY tb_page_" . $tableName . ".tb_page_" . $tableName . "_title");
            return $Q->result();
        }
        return null;
    }
    ...
    public static function getCounter() {
        $_this = & get_Instance();
        $_this->db->select_sum('counter');
        $t = $_this->db->get('visitors')->row_array();
        return $t['counter'];
    }

    public static function widgetverticals($ids, $active = '1') {
        $_this = & get_Instance();
        $_this->db->trans_start();
        $active = ($active == 'all' ? "" : "AND widget_active ='1' ");
        if (empty($dynamicPosition)) {
		...
		SELECT 
			tb_widgets.*
		FROM tb_widgets WHERE 1=1 " . $active . "  AND tb_widgets.widget_id IN ({$ids})
		GROUP BY tb_widgets.widget_id ORDER BY widget_ordering			
		");
        }
        $_this->db->trans_complete();
